...
import argparse
import regularization_conv_nn as nn
import os
...
# run inference on the input data
  x = tf.placeholder(tf.float32, shape=(batch_size, img_size[0], img_size[1], img_size[2], img_size[3]))
...
# setup the training operations
  ...
  with tf.Session() as sess:
    ...
    numsteps = int(len(valid_dataset)/batch_size)
    accuracy_total = 0
    for step in range(numsteps):
      ...
      accuracy_total+=accuracy[0]
      print('OUTPUT: Step %d: accuracy = %.3f' % (step, accuracy[0]))

    numsteps_test = int(len(test_dataset)/batch_size)
    accuracy_total_test = 0
    for step in range(numsteps_test):
      ...
      accuracy = sess.run([accuracy_eval], feed_dict={x: batch_data, y_: batch_labels, keep_prob: 1})
      accuracy_total_test+=accuracy[0]
      print('OUTPUT: Step %d: accuracy = %.3f' % (step, accuracy[0]))        

    print('OUTPUT: Average accuracy = %.3f' % (accuracy_total/numsteps))
    print('OUTPUT: Average accuracy = %.3f' % (accuracy_total_test/numsteps_test))
